...
export const useTextmenuCommands = (editor) => {
  ...
  const onTone = useCallback((tone) => editor.chain().focus().aiAdjustTone(tone).run(), [editor]);
  const onTranslate = useCallback((language) => editor.chain().focus().aiTranslate(language).run(), [editor]);
